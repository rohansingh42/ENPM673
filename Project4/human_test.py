...
def warpAll(t,I,p,Igradx,Igrady):
    ...
    for i in range(t.shape[1]):            # x
        for j in range(t.shape[0]):       # y
            ...
            # print(W.shape)
            # W = np.int64(W)
            # if (W[1] >= t.shape[0]) | (W[0] >= t.shape[1]):
            #     continue
            a = I[W[1],W[0]]
            igx = Igradx[W[1],W[0]]
            igy = Igrady[W[1],W[0]]
    ...
    while (delpnorm > thresh):
        ...
        for i in range(t.shape[1]):#xl,xu):            # x
            for j in range(t.shape[0]):#yl,yu):       # y
                ...
                # print('wJ',wJ.shape)
                # print('ig',ig.shape)
                if np.absolute(diff[j,i]) < k:
                    weight = 1
                else:
                    weight = k/np.absolute(diff[j,i])
                b1 = np.matmul(ig,wJ)
                b2 = diff[j,i]
...
img1 = cv2.imread('./data/human/0140.jpg')
# temp = np.zeros([60,80])
xl = 259
xu = 283
yl = 295
...
cv2.imwrite('./data/human_template.jpg',temp)
...
while True:
    if frno < 100:
        frame = cv2.imread('./data/human/00'+str(frno)+'.jpg')
    else:
        frame = cv2.imread('./data/human/0'+str(frno)+'.jpg')
    ...
    # cv2.imshow('I',I)
    cv2.imwrite('./data/human_modtry6/hm' + str(frno) + '.jpg',frame)
    cv2.imwrite('./data/human_modtry6/warpedhm' + str(frno) + '.jpg',wIprev)
    print('frame',frno)

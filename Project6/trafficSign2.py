...
imageList.sort()
imageList=imageList[90:]

kk=90
for jj,img in enumerate(imageList):
    ...
    mask=np.ma.greater(cDashRed,0.25)
    ...
    mser = cv2.MSER_create(_delta=1,_min_diversity = 0.2,_min_area=600,_max_area=2000,_max_variation=0.15)
    grayBlue =cDashBlue
    grayRed=cDashRed
    vis = firstImage.copy()
                ...
                if(h<3*w):
                    ...
                    if(y-buff>10 and x-buff>10):
                        ...
                        if(pred==45 and predAcc>acc):
                            ...
                            if(x-buff-40>10 and x+w+buff-40>10 ):
                                vis[y-buff:y+h+buff,x-buff:x+w+buff,:]=sign
                                cv2.rectangle(vis,(x-buff,y-buff),(x+w+buff,y+h+buff),(0,255,0),2)
        ...
        if(h>=0.7*w):
            if(h<1.2*w):
                ...
                #                             if(x-40>10 and x+w-40>10 ):
                        ...
                        if(x-buff-40>20 and x+w+buff-40>20 ):
                                vis[y-buff:y+h+buff,x-buff:x+w+buff,:]=sign
                                cv2.rectangle(vis,(x-buff,y-buff),(x+w+buff,y+h+buff),(0,255,0),2)
...
#                cv2.rectangle(vis,(x-buff,y-buff),(x+w+buff,y+h+buff),(0,255,0),2)
    ...
    cv2.imwrite("results/det_and_class/"+str(kk)+'.jpg',vis)
    kk+=1
    #cv2.imshow('lol',vis)
